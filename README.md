...
- Search students by name (server-side prefix query with debounce)
...
### `GET /api/health`
Health check. Returns `{ ok, service, time, firebase, gemini }`. Returns `503` if either `NEXT_PUBLIC_FIREBASE_PROJECT_ID` or `GEMINI_API_KEY` is missing.
...
| `academic` | array | no | Academic records (GPA must be 0–10) |
...
44 tests across 7 suites, all passing.
