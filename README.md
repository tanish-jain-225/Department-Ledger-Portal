...
[![Tests](https://img.shields.io/badge/Tests-55_passing-brightgreen?style=flat)](__tests__)
...
Returns service health and environment readiness:

- ok
- service
- time
- firebase status
- gemini api/model status
...
- Rate limiting uses in-memory sliding window (lib/rate-limit.js) with sanitized key handling. For multi-instance deployments, replace with shared store (e.g., Redis).

## Testing

Current status: 55 tests, 9 suites, all passing.
...
All three commands pass on the current codebase.
