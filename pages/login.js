        ...
        <div className="premium-card p-10 sm:p-14 relative overflow-hidden">
          ...
          {resetSent && (
            <div className="mb-8 flex gap-4 rounded-3xl bg-brand-50 border border-brand-100 p-6 animate-slide-up">
              ...
              <p className="text-sm text-brand-800 font-bold leading-relaxed">Password reset link sent. Check your inbox.</p>
            </div>
          )}
          ...
          {registered && (
            <div className="mb-8 flex gap-4 rounded-3xl bg-emerald-50 border border-emerald-100 p-6 animate-slide-up">
              ...
              <p className="text-sm text-emerald-800 font-bold leading-relaxed">
                Account created. Please await administrative role assignment for full access.
              </p>
            </div>
          )}
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] pl-1 mb-2 block">Email</label>
              <Input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Institutional Email" />
            </div>
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] pl-1 mb-2 block">Password</label>
              <div className="relative">
                <Input type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••••••" className="pr-14" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-brand-500 transition-colors">
                  {showPassword ? (
            ...
            <div className="text-center">
              {!resetMode ? (
                <button type="button" onClick={() => { setResetMode(true); setErr(""); }} className="text-xs font-bold text-slate-400 hover:text-brand-600 transition-colors uppercase tracking-widest">
                  Forgot password?
                </button>
              ) : (
                <div className="flex items-center justify-center gap-3">
                  <button type="button" onClick={onReset} disabled={resetBusy} className="text-xs font-black text-brand-600 hover:text-brand-700 transition-colors uppercase tracking-widest">
                    {resetBusy ? "Sending..." : "Send reset link"}
                  </button>
                  <span className="text-slate-200">|</span>
                  <button type="button" onClick={() => { setResetMode(false); setErr(""); }} className="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest">
                    Cancel
                  </button>
                </div>
              )}
          ...
          <p className="mt-10 text-center text-sm font-black text-slate-400 uppercase tracking-widest">
            No account?{" "}
            <Link href="/register" className="text-brand-600 hover:text-brand-700 transition-colors">Register</Link>
          </p>
        </div>
      </div>
    </Layout>
