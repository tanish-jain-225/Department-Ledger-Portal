    ...
    <Layout title="Governance Overview" access={ACCESS.ADMIN}>
      <div className="space-y-10 animate-slide-up">
        <div>
          ...
          <p className="text-base text-slate-400 mt-2 font-medium">Real-time surveillance and administrative metrics for the ledger.</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {stats.recentAudits.map(log => (
                  <div key={log.id} className="p-5 rounded-2xl bg-slate-50/50 border border-slate-100/50 flex items-center justify-between gap-6 transition-all hover:bg-white hover:shadow-md">
                    <div className="min-w-0">
                      <p className="text-sm font-black text-slate-800 line-clamp-1">{log.description}</p>
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">
                        {new Date(log.timestamp?.seconds * 1000).toLocaleString()}
                      </p>
                    </div>
                    <Badge variant="neutral" className="bg-slate-900 text-white border-none py-1.5">
          ...
          {/* Quick Actions */}
          <div className="premium-card p-8 bg-slate-900 text-white border-none shadow-2xl shadow-slate-900/40 relative overflow-hidden">
            ...
            <div className="mt-12 pt-8 border-t border-white/10 opacity-60">
               <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">System Precision</p>
               <p className="text-xs font-medium mt-2 leading-relaxed italic">&ldquo;Administering the department with zero-trust security and high-fidelity record keeping.&rdquo;</p>
            </div>
          </div>
        </div>
      </div>
