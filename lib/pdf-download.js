...
export async function downloadElementAsPdf(element, options = {}) {
  ...
  // Keep cloned content within printable A4 bounds (10mm margins from globals.css).
  const safeMaxWidth = orientation === 'landscape' ? 1020 : 700;
  const targetWidth = Math.min(windowWidth, safeMaxWidth);
  ...
  if (orientation === 'landscape') {
    clone.classList.add('print-page-landscape');
  }
  ...
  if (orientation === 'landscape') {
    document.body.classList.add('isolate-print-landscape');
  }
  ...
  try {
    ...
    document.body.classList.remove('isolate-print-landscape');
