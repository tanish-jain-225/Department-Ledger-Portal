...
function shouldUseSharedStore() {
  ...
  // Bypasses firebase-admin Firestore rate limiting if credentials are missing
...
async function isRateLimitedShared(safeKey, max, windowMs) {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(() => reject(new Error("Shared rate limiter request timed out")), 1500);
  });
