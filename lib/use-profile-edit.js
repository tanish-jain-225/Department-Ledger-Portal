...
export function useProfileEdit(uid, email, name, onRefresh) {
  ...
  const saveProfile = useCallback(async (fields) => {
    ...
    try {
      ...
      // Defensively sanitize fields: strip out privileged role fields to prevent unauthorized elevation or self-demotion
      const { role: _r, facultyVerification: _fv, ...safeFields } = fields || {};
      await updateDoc(doc(db, "users", uid), { ...safeFields, updatedAt: serverTimestamp() });
      await onRefresh();
      await logAudit({
  ...
  const requestDeletion = useCallback(async () => {
    ...
    try {
      await createRecord("deletionRequests", {
