...
export async function logAudit({ action, actorUid, targetUid, description, details }) {
  ...
  try {
    ...
    // Intentionally swallowed — an audit write failure must never block or
    // roll back the user's triggering action. Log to server-side surveillance only.
    console.error(`[Audit Service] Stream interruption: ${e.message}`);
  }
}
