import {
  addDoc,
  collection,
  deleteDoc,
  doc,
  getDoc,
  getDocs,
  limit,
  query,
  serverTimestamp,
  updateDoc,
  where,
  orderBy,
  startAfter,
  writeBatch,
} from "firebase/firestore";
import { getDb } from "./firebase";
import { logAudit } from "./audit";
import { PAGE_SIZE } from "./constants";

async function safeLogAudit(payload, context) {
  try {
    await logAudit(payload);
  } catch (error) {
    const message = String(error?.message || "").toLowerCase();
    const isPermissionIssue = message.includes("insufficient permissions") || message.includes("permission-denied");
    if (!isPermissionIssue) {
      console.warn(`[Audit] ${context}: ${error.message}`);
    }
  }
}

/**
 * Helper to get a Firestore collection reference.
 * @param {string} name - Collection name.
 * @returns {import("firebase/firestore").CollectionReference}
 * @throws {Error} If Firestore is not initialized.
 */
export function col(name) {
  const db = getDb();
  if (!db) throw new Error("[Firestore] Service not initialized or available.");
  return collection(db, name);
}

/**
 * Retrieves a list of records for a specific student, sorted by creation date.
 * Implements a graceful fallback for missing Firestore composite indexes.
 * 
 * @param {string} collectionName - Target Firestore collection.
 * @param {string} studentUid - The UID of the student.
 * @param {number} [pageSize=20] - Number of records per page.
 * @param {import("firebase/firestore").DocumentSnapshot} [lastDoc=null] - For pagination.
 * @returns {Promise<Array<Object>>}
 */
export async function listByStudent(collectionName, studentUid, pageSize = PAGE_SIZE.DEFAULT, lastDoc = null) {
  const db = getDb();
  if (!db) return [];
  
  const baseConstraints = [where("studentUid", "==", studentUid)];
  
  const constraints = [...baseConstraints, orderBy("createdAt", "desc"), limit(pageSize)];
  if (lastDoc) constraints.push(startAfter(lastDoc));
  
  const q = query(collection(db, collectionName), ...constraints);
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

export async function listStudentDocuments(studentUid, pageSize = PAGE_SIZE.DEFAULT) {
  const db = getDb();
  if (!db) return [];
  const q = query(
    collection(db, "uploadedDocuments"),
    where("studentUid", "==", studentUid),
    orderBy("createdAt", "desc"),
    limit(pageSize)
  );
  const snap = await getDocs(q);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

/**
 * Administrative query tool for student dashboards with multi-parameter filtering.
 *
 * Search strategy:
 *   - When `search` is provided, uses a Firestore prefix range query on the
 *     `name` field (orderBy name, startAt term, endAt term + "\uf8ff") so the
 *     filter runs server-side and scales with the collection size.
 *   - Branch / year equality filters are applied as additional `where` clauses.
 *
 * @param {Object} params - Query parameters.
 * @param {string} [params.branch] - Department branch.
 * @param {string} [params.year] - Academic year.
 * @param {string} [params.search] - Prefix search term (matched against name).
 * @param {number} [params.pageSize=50] - Records per page.
 * @param {import("firebase/firestore").DocumentSnapshot} [params.lastDoc=null] - Firestore cursor for pagination.
 * @returns {Promise<{ rows: Array<Object>, lastDoc: import("firebase/firestore").DocumentSnapshot|null }>}
 */
export async function listStudentsForDashboard({
  branch = "",
  year = "",
  search = "",
  pageSize = PAGE_SIZE.DASHBOARD,
  lastDoc = null,
}) {
  const db = getDb();
  if (!db) return { rows: [], lastDoc: null };

  const usersRef = collection(db, "users");
  const term = search.trim();

  let constraints = [where("role", "==", "student")];

  if (branch) constraints.push(where("branch", "==", branch));
  if (year) constraints.push(where("year", "==", year));

  if (term) {
    // Server-side prefix range query (name or email)
    const end = term + "\uf8ff";
    const field = term.includes("@") ? "email" : "name";
    constraints = [
      ...constraints,
      orderBy(field),
      where(field, ">=", term),
      where(field, "<=", end),
    ];
  }

  if (lastDoc) constraints.push(startAfter(lastDoc));
  constraints.push(limit(pageSize));

  try {
    const q = query(usersRef, ...constraints);
    const snap = await getDocs(q);
    const rows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    const cursor = snap.docs.length > 0 ? snap.docs[snap.docs.length - 1] : null;
    return { rows, lastDoc: cursor };
  } catch (error) {
    const code = String(error?.code || "").toLowerCase();

    // Fallback for missing composite indexes in dev/staging:
    // page through simpler server-side batches and filter in-memory.
    if (term && code === "failed-precondition") {
      const lowered = term.toLowerCase();
      const rows = [];
      let cursor = lastDoc;
      let hasMore = true;
      let safetyCounter = 0;

      while (rows.length < pageSize && hasMore && safetyCounter < 10) {
        const fallbackConstraints = [where("role", "==", "student")];
        if (branch) fallbackConstraints.push(where("branch", "==", branch));
        if (year) fallbackConstraints.push(where("year", "==", year));
        if (cursor) fallbackConstraints.push(startAfter(cursor));
        fallbackConstraints.push(limit(pageSize));

        const fallbackQ = query(usersRef, ...fallbackConstraints);
        const fallbackSnap = await getDocs(fallbackQ);

        const batch = fallbackSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        rows.push(
          ...batch.filter((r) => {
            const name = String(r?.name || "").toLowerCase();
            const email = String(r?.email || "").toLowerCase();
            return name.includes(lowered) || email.includes(lowered);
          })
        );

        hasMore = fallbackSnap.docs.length === pageSize;
        cursor = fallbackSnap.docs.length > 0 ? fallbackSnap.docs[fallbackSnap.docs.length - 1] : cursor;
        safetyCounter += 1;
      }

      const sliced = rows.slice(0, pageSize);
      return { rows: sliced, lastDoc: cursor || null };
    }

    throw error;
  }
}

/**
 * Commits a new record to the ledger with automatic timestamping and optional auditing.
 * 
 * @param {string} collectionName - Target collection.
 * @param {Object} data - Record data.
 * @param {Object} [logging=null] - Audit metadata.
 * @returns {Promise<string>} The new document ID.
 */
export async function createRecord(collectionName, data, logging = null) {
  try {
    const ref = await addDoc(col(collectionName), {
      ...data,
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
    });
    
    if (logging) {
      await safeLogAudit({
        action: `${collectionName}_created`,
        actorUid: logging.actorUid,
        targetUid: data.studentUid || logging.targetUid || null,
        description: logging.description || `Created new record in ${collectionName}`,
        details: { id: ref.id, ...data }
      }, `Failed to write ${collectionName}_created`);
    }
    
    return ref.id;
  } catch (error) {
    throw new Error(`[Data Layer] Creation failed in ${collectionName}: ${error.message}`);
  }
}

/**
 * Updates an existing record with optional auditing.
 * 
 * @param {string} collectionName - Target collection.
 * @param {string} id - Document ID.
 * @param {Object} data - Update data.
 * @param {Object} [logging=null] - Audit metadata.
 */
export async function updateRecord(collectionName, id, data, logging = null) {
  const db = getDb();
  if (!db) return;
  
  try {
    await updateDoc(doc(db, collectionName, id), {
      ...data,
      updatedAt: serverTimestamp(),
    });

    if (logging) {
      await safeLogAudit({
        action: `${collectionName}_updated`,
        actorUid: logging.actorUid,
        targetUid: data.studentUid || logging.targetUid || null,
        description: logging.description || `Updated record ${id} in ${collectionName}`,
        details: { id, ...data }
      }, `Failed to write ${collectionName}_updated`);
    }
  } catch (error) {
    throw new Error(`[Data Layer] Update failed in ${collectionName}: ${error.message}`);
  }
}

/**
 * Purges a record from the ledger with optional auditing.
 * 
 * @param {string} collectionName - Target collection.
 * @param {string} id - Document ID.
 * @param {Object} [logging=null] - Audit metadata.
 */
export async function removeRecord(collectionName, id, logging = null) {
  const db = getDb();
  if (!db) return;
  
  try {
    // Cascade-delete associated uploaded files to prevent orphaned database records
    if (collectionName !== "uploadedDocuments") {
      try {
        const recordSnap = await getDoc(doc(db, collectionName, id));
        if (recordSnap.exists()) {
          const recordData = recordSnap.data();
          const docId = recordData?.document?.documentId || 
                        recordData?.uploadedDocument?.documentId || 
                        recordData?.documentId;
          if (docId) {
            await deleteDoc(doc(db, "uploadedDocuments", docId));
          }
        }
      } catch (docError) {
        console.warn(`[Data Layer] Orphan document cleanup failed for record ${id}:`, docError);
      }
    }

    if (logging) {
      await safeLogAudit({
        action: `${collectionName}_deleted`,
        actorUid: logging.actorUid,
        targetUid: logging.targetUid || null,
        description: logging.description || `Deleted record ${id} from ${collectionName}`,
        details: { id }
      }, `Failed to write ${collectionName}_deleted`);
    }
    await deleteDoc(doc(db, collectionName, id));
  } catch (error) {
    throw new Error(`[Data Layer] Removal failed in ${collectionName}: ${error.message}`);
  }
}

/**
 * Permanently purges a user and all their sub-collection records from the ledger.
 * Consolidates the cascade-delete logic used in admin/students.js and admin/requests.js.
 *
 * @param {string} targetUid  - UID of the user to delete.
 * @param {string} actorUid   - UID of the admin performing the action.
 * @param {string} [description] - Optional audit description override.
 */
export async function purgeUser(targetUid, actorUid, description) {
  const db = getDb();
  if (!db) throw new Error("[Firestore] Service not initialized.");

  // Governance policy: administrators cannot delete their own account
  if (targetUid === actorUid) {
    throw new Error("Governance policy violation: Self-deletion is forbidden. An administrator cannot delete their own account.");
  }

  const { USER_SUB_COLLECTIONS } = await import("./constants");
  const allRefs = [];

  // 1. Gather all sub-collection document references for deletion
  for (const collName of USER_SUB_COLLECTIONS) {
    const q = query(collection(db, collName), where("studentUid", "==", targetUid));
    const snap = await getDocs(q);
    snap.docs.forEach((d) => allRefs.push(d.ref));
  }

  // 1b. Gather notifications, role requests, and deletion requests for deletion
  const qNotif = query(collection(db, "notifications"), where("userUid", "==", targetUid));
  const snapNotif = await getDocs(qNotif);
  snapNotif.docs.forEach((d) => allRefs.push(d.ref));

  for (const collName of ["roleRequests", "deletionRequests"]) {
    const q = query(collection(db, collName), where("uid", "==", targetUid));
    const snap = await getDocs(q);
    snap.docs.forEach((d) => allRefs.push(d.ref));
  }

  // 2. Append the main user document reference
  allRefs.push(doc(db, "users", targetUid));

  // 3. Commit deletions in chunked batches of 400 to avoid Firestore's 500-write limit
  const batchSize = 400;
  for (let i = 0; i < allRefs.length; i += batchSize) {
    const chunk = allRefs.slice(i, i + batchSize);
    const batch = writeBatch(db);
    chunk.forEach((ref) => batch.delete(ref));
    await batch.commit();
  }

  // 4. Write immutable audit entry
  await logAudit({
    action: "user_deleted",
    actorUid,
    targetUid,
    description: description || `Permanently purged user entity and all records for UID: ${targetUid}`,
    details: { collections: USER_SUB_COLLECTIONS },
  });
}
