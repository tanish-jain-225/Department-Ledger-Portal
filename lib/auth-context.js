...
import { doc, getDoc, getDocs, query, collection, where, serverTimestamp, setDoc, updateDoc, writeBatch } from "firebase/firestore";
...
export function AuthProvider({ children }) {
  ...
  const register = useCallback(
    async ({ email, password, name, rollNumber, year, branch }) => {
      setError(null);
      const auth = getFirebaseAuth();
      const db = getDb();
      if (!auth || !db) throw new Error("[Auth Engine] Infrastructure not initialized.");
      ...
      try {
        // 1. Create Authentication Identity (Required for Firestore 'isAuth()' rules)
        ...
        // 2. Pre-flight Check: Uniqueness Guard (Now authenticated)
        if (cleanRoll) {
          try {
            const q = query(collection(db, "users"), where("rollNumber", "==", cleanRoll));
            const snap = await getDocs(q);
            if (!snap.empty) {
              throw new Error(`Security Exception: Roll Number ${cleanRoll} is already registered in the official ledger.`);
            }
          } catch (err) {
            ...
            console.warn("[Auth Context] Roll number uniqueness check skipped due to permission rules.");
          }
        }
        
        // 3. Atomic Batch: Profile + Governance Request
        const isFaculty = year === "faculty";
        const isAlumni = year === "alumni";
        const cleanYear = isFaculty || isAlumni ? "" : (year || "");
        ...
        const roleReqRef = doc(collection(db, "roleRequests"));
        batch.set(roleReqRef, {
          uid,
          email: cleanEmail,
          rollNumber: cleanRoll,
          requestedRole: isFaculty ? "faculty" : "student",
          status: "pending",
          createdAt: serverTimestamp(),
        });
        ...
        // 5. Self-Healing: Clean up broken auth accounts (Zombie Account Prevention)
        if (newUser) {
          try {
            await deleteUser(newUser);
          } catch (deleteErr) {
            console.error("[Auth Context] Critical: Failed to clean up broken auth account:", deleteErr.message);
          }
        }
        
        // Map Firebase Auth errors to professional messages
        const msg = err.code === 'auth/email-already-in-use' 
          ? "Identity Conflict: This email address is already registered."
          : err.message;
  ...
  const logout = useCallback(async () => {
    ...
    try {
      await signOut(auth);
      // eslint-disable-next-line @next/next/no-location-assign-relative-destination -- Intentional hard reload flushes all in-memory React and Firebase auth state
      window.location.href = "/";
    } catch (err) {
      console.error("Logout Error:", err);
    } finally {
      // Always reset the overlay flag. On success the page redirect fires first,
      // but the finally ensures no stuck overlay state in future refactors.
