...
export async function buildStudentPdf(student, lists, report) {
  ...
  function drawFieldBlock(fields, startX, endX, startY) {
    ...
    fields.forEach(([label, value]) => {
      ...
      fy += fieldLineH + (wrapped.length > 1 ? 8 : 0);
    });
    return fy;
  }
  ...
  if (docs.length === 0) {
    ...
    doc.setTextColor(100, 116, 139);
    doc.text("#",          LEFT + 4,   y);
    doc.text("FILE NAME",  LEFT + 20,  y);
    doc.text("TYPE",       LEFT + 270, y);
    doc.text("SIZE",       LEFT + 350, y);
    doc.text("UPLOADED",   LEFT + 420, y);
    y += 10;
    ...
    docs.forEach((docItem, idx) => {
      ...
      const nameTruncated = rawName.length > 42 ? rawName.slice(0, 39) + "..." : rawName;
      doc.text(nameTruncated, LEFT + 20, y);

      // MIME type simplified
      const mime = (docItem.mimeType || "").replace("application/", "").replace("image/", "IMG/").toUpperCase() || "DOC";
      doc.text(mime.slice(0, 12), LEFT + 270, y);
      ...
      doc.text(uploadDate, LEFT + 420, y);
  ...
  // Ensure footer is always at bottom of page 1
  const coverFooterY = PAGE_H - 28;
  doc.setDrawColor(226, 232, 240);
  doc.setLineWidth(0.5);
  doc.line(LEFT, coverFooterY, RIGHT, coverFooterY);
  doc.setFont("helvetica", "italic");
  doc.setFontSize(7);
  doc.setTextColor(148, 163, 184);
  doc.text("Official Department Academic Ledger Report  ·  Cryptographically Secured  ·  Page 1 of Summary", LEFT, coverFooterY + 10);
  doc.setFont("helvetica", "normal");
  doc.text("Seal of Academic Registry Office", RIGHT, coverFooterY + 10, { align: "right" });
  ...
  if (lists.academic && lists.academic.length > 0) {
    ...
    lists.academic.forEach((sem) => {
      ...
      doc.text(wrappedDetails[0] || "-", LEFT + 140, y);
      y += 14;
    });
  } else {
    doc.setFont("helvetica", "italic");
  ...
  if (lists.projects && lists.projects.length > 0) {
    lists.projects.forEach((proj) => {
      ...
      if (proj.description) {
        ...
        doc.setFontSize(8.5);
        doc.setTextColor(71, 85, 105);
        const wrappedDesc = doc.splitTextToSize(proj.description, WIDTH);
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(148, 163, 184);
  doc.text("Official Department Academic Ledger Report  ·  Cryptographically Secured", LEFT, y);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.5);
  doc.setTextColor(100, 116, 139);
  doc.text("Signature / Seal of Academic Registry Office", RIGHT, y, { align: "right" });
